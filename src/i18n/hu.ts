export default {
  metaDescription: "Simon Júlia web fejlesztő",
  nav: {
    home: "Főoldal",
    blog: "Digitális kert",
    about: "Rólam",
    projects: "Projektek"
  },
  digitalGarden: {
    definition: "A digitális kert folyamatosan fejlődő jegyzetek, ötletek és források nemlineáris gyűjteménye. Abban különbözik a blogoktól, hogy kevésbé szervezett, és idővel változhat, akár egy kert.",
    introduction: "Nagyon szeretek új dolgokat tanulni, és arra gondoltam, hogy készítek néhány jegyzetet a tanultakról jövőbeli énemnek vagy más olyan fejlesztőknek, akik ezeket érdekesnek találhatják."
  },
  about: {
    degree: "Kognitív pszichológiából szereztem PhD fokozatot, és nagyon szerettem az alapkutatást. Idővel azonban egyre inkább szükségét éreztem annak, hogy valami kézzelfoghatót hozzak létre.",
    programmingRoots: "A programozás mindig közel állt hozzám: már az alapképzésem alatt is próbáltam kis játékokat készíteni. Később kísérleti feladatok megjelenítéséhez és adatelemzéshez használtam.",
    programmingWhy: "Aztán rátaláltam a webfejlesztésre, egy olyan területre, amely sok érdeklődési körömhöz kapcsolódik: problémamegoldás, vizuális művészet és az emberi viselkedés.",
    experience: "Az elmúlt öt évben (2021-2026) fantasztikus csapatokban dolgoztam (Warner Brothers, Warner Brothers and Discovery, a Mindtechapps belső csapatai, IESF, Mazed, Spotify). Mindig hálás leszek azoknak a vezetőknek, akiktől rengeteget tanultam (teljesség igénye nélkül): Lei, Mikael Ek, Légár Eszter, Volosin Márton, Tóth Dávid és Néninger Milán.",
    choiceOfWeapon: "Választott eszközeim:",
    hobbies: "Hobbik: kertészkedés, kutyázás, növényi alapú receptekkel való kísérletezés, olvasás"
  },
  projects: {
    description: "Néhány az első hobby projektjeim közül, amikor elkezdtem webfejlesztéssel foglalkozni.",
    persevereDescription: "Célokat tűzhetsz ki magadnak, és növelheted a megszakítás nélküli elköteleződés pontszámod azzal, hogy minden nap bejelentkezel és bejelölöd a napot (ha dolgoztál rajta). Teszteléshez használhatod a 'test@gmail.com' fiókot a 'TestTest2' jelszóval.",
    plastic3matchDescription: "Csak az újrahasznosítható műanyag darabokat tudod kombinálni és eltávolítani, kivéve ha új technológiát fejlesztesz.",
    balloonDescription: "Emeld fel a léggömböt úgy, hogy háromszor egymás után helyesen különbözteted meg az egymást követő hangok sorrendjét. (Lehetséges, hogy az irányított hangok nem működnek.)"
  },
  footer: {
    copyright: "- © Simon Júlia 2021 -"
  }
};