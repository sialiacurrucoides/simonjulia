...
### Rendering tab of Chrome developer tools
 Ha nem tudod hogy hozd elő, "Command + Shift + P"-vel rákereshetsz a _rendering_ kifejezésre. Itt például az _Emulate vision deficiencies_ rész segítségével meg tudod nézni, hogy milyen típusú látási nehézségnél, hogy néz ki az oldalad. A Chrome-ban van egy Accessibility tab is, ami szintén hasznos lehet.
<figure>
...
Az oldal akadálymentességét tesztelni lehet a Lighthouse futtatásával, és visszejelzéseket kapunk, hogy mit lenne érdemes javítani. Érdemes azonban tudni, hogy ha egy logikailag fontos elemünk nem fókuszálható, nem biztos, hogy jelzi nekünk, mert dekorációnak tekinti. Így nem helyettesíti a kézi ellenőrzést.
...
- Régi történet, de igyekezzünk a szemantikus tag-eket használni. Ha gombot szeretnénk létrehozni, formázzuk a _button_ tag-et, ne egy _div_-et. Hiába tesszük fókuszálhatóvá egy tabindex-el, attól még nem lesz automatikusan kattintható. Továbbá a képernyő olvasóknak van gyors menüje, ami például a külöböző _h_ tageken keresztül tud ugrálni.
- A címeknél tartsuk be a hierarchikus sorrendet a h tagekek használatánál. A h1 után ne használjunk egyből h4-et, csak mert nem akarunk nagy betűket. Inkább formázzuk a h2-t.
...
- Ellenőrizzük, hogy tab használattal kimegyünk-e a képernyőből. Például hamburger menü után sok tab nyomásig nem látunk semmi fókuszált elemet, mert bár nem nyitottuk meg a menüt, a tab végighalad az elemein.
- Ne vegyük le az "outline"-t, ha nem kezejük másképpen a fokuszált állapotot, például a :focus-visible pseudo-class segítségével.
- Tegyünk tabindex=0-t a nem fókuszált, de fókszálandó elemekre, de ne használjunk magasabb értéket, mert az anti-pattern.
...
- Időnként ellenőrizzük a honlapunk a fenti eszközökkel és javítsuk az előkerülő hiányosságokat. Ma már nem kell fejben tartanunk minden szintaxtis, mert hamar megkereshető a megoldás. A problémát azonban nekünk kell megtalálnunk.
- A figyelmeztető elemeket (alert), helyezzük a rögtön a body tag nyitása után, hogy könnyebben meg lehessen találni őket a kisegítő technológiákkal. Ha nem életbevágó, használjuk az aria-live="polite" beállítást, hogy ne legyen túl intrúzív a figyelmeztetés.
...
- Adjuk meg az oldal nyelvét: `<html lang="en">`
...
- A fontos kontrol elemek legyenek elöl az oldalon, hogy ne kellejen végignavigálni a teljes oldalt, hogy elérhessük. Például a kosár legyen a felső menüben, ne alul.
...
- Hanganyag mellé tervezzünk elérhető feliratot vagy átiratot. Elengáns megoldás lehet, ha a videó mellett van egy fal, ahol megjelenik a szöveg és mindig ki van emelve az a rész, ami éppen elhangzik. Az egyszerű felirathoz képest, itt több a kontextus, ha valaki lassabban olvas, még láthatóak a korábbi mondatok.
